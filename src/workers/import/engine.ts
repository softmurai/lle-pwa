...
export async function runImport(db: D1Database, csvText: string): Promise<ImportReport> {
  ...
  const matchId = async (week: string, home: number, away: number, date: string, homeScore: number, awayScore: number, status: string): Promise<number> => {
    const key = `${week}|${home}|${away}`;
    if (matchIds.has(key)) return matchIds.get(key)!;
    await db
      .prepare(
        'INSERT INTO matches (week, date, home_team_id, away_team_id, home_score, away_score, status) VALUES (?, ?, ?, ?, ?, ?, ?) ON CONFLICT(week, home_team_id, away_team_id) DO UPDATE SET date = excluded.date, home_score = excluded.home_score, away_score = excluded.away_score, status = excluded.status'
      )
      .bind(week, date, home, away, homeScore, awayScore, status)
      .run();
  ...
  for (let r = 1; r < rows.length; r++) {
    ...
    const match = await matchId(week, homeId, awayId, date, homeScore, awayScore, status);
  ...
  const ins = db.prepare(
    `INSERT INTO standings (team_id, played, wins, losses, points_for, points_against)
     SELECT team_id, COUNT(*) AS played, SUM(wins) AS wins, SUM(played - wins) AS losses, SUM(points_for) AS points_for, SUM(points_against) AS points_against
     FROM (
